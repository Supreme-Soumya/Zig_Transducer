 ...
 * CONDITIONS OF ANY KIND, either express or implieda.
 */
#include <fcntl.h>
#include <string.h>
...
static esp_err_t zb_action_handler(esp_zb_core_action_callback_id_t callback_id, const void *message)
{
    if (callback_id == ESP_ZB_CORE_SET_ATTR_VALUE_CB_ID) {
        ...
        if (msg->info.cluster == CUSTOM_CLUSTER_ID &&
            ...
            // ZCL string: first byte is length
            uint8_t *zcl_str = (uint8_t *)msg->attribute.data.value;
            uint8_t str_len  = zcl_str[0];

            // Copy to null-terminated buffer for printing/parsing
            char json_buf[CUSTOM_JSON_MAX_LEN] = {0};
            memcpy(json_buf, zcl_str + 1, str_len);
            json_buf[str_len] = '\0';

            ESP_LOGI("COORDINATOR", "Received JSON: %s", json_buf);

            // Parse individual values if needed
            // You can use sscanf or cJSON library here
            float s1, s2, s3;
            if (sscanf(json_buf,
                "{\"sensor1\":%f,\"sensor2\":%f,\"sensor3\":%f}",
                &s1, &s2, &s3) == 3) {
                ESP_LOGI("COORDINATOR", "sensor1=%.1f sensor2=%.1f sensor3=%.1f",
                         s1, s2, s3);
            }
        }
    }
